...
function makeItem(meta,title,link,pubDate='',description='',source=''){const declaredTrust=meta.trust||'B';const effectiveTrust=meta.mode.includes('fallback')&&declaredTrust==='A'?'B':declaredTrust;const x={market:meta.market||'全球',signal:meta.signal||'事实',type:inferType(title,meta.type||''),title,link,pubDate,description:description.slice(0,500),source:source||meta.name,sourceKey:meta.key,sourceMode:meta.mode,declaredTrust,trustTier:effectiveTrust,confidence:effectiveTrust};x.verification=x.signal==='热点'?'热点发现：不得直接发布，必须回到一级信源核验':x.trustTier==='A'?'一级/官方信源候选：发布前核对原文关键数字与日期':'发现或降级候选：需匹配公司公告、监管文件或政府原文';x.score=score(x);x.priority=priority(x.score);x.id=crypto.createHash('sha1').update(`${x.sourceKey}|${x.title}|${x.link}`).digest('hex').slice(0,16);return x}
